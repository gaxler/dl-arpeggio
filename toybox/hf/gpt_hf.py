...
from arpeggio.transformers.model import GPT, GPTTokenSeq
...
from dataloader import HuggingFaceTask

HuggingFaceDataset = Any
""" Can't find proper type annotation in HF datasets. for now """
...
@eqx.filter_jit
def step(
    x: Int[GPTTokenSeq, "bsize"], mask, prng_key, model, opt_state, param_update_fn
):
    """
    forward pass:
        * comp loss and grads ->
...
class GPTTrainer:
    ...
    def predict(self, tokens: Int[GPTTokenSeq, "bsize"]) -> Tuple[Array, Array]:
        """
    ...
    def _gen(self, idxs: GPTTokenSeq):
        return self.local_model.generate(
            idxs, max_pred_tokens=idxs.shape[0], key=None  # self._rng_key()
        )

    def gen_from_tokens(self, promt_idxs: GPTTokenSeq):
...
def train(
    trainer_conf: TrainerConf, prng_key: jax.random.PRNGKey 
) -> GPT:
    ...
    hf_data = HuggingFaceTask.gpt_tokenizer_from_dataset(
        name_or_path=trainer_conf.hf_dataset_name, split="train"
    )

    batch_size = trainer_conf.batch_size
    dataloader = hf_data.get_dataloader(batch_size=batch_size)

    txt_encode, token_decode = hf_data.txt_enc, hf_data.token_decode
    ...
    def _generation_printout(epoch_num):
        ...
        gen_tokens = [trainer.gen_from_tokens(txt_encode(txt)) for txt in prompts]
