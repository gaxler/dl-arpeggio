...
class SortingTask:
    ...
    def get_dataloader(self, batch_size: int, random_seq_len: float = 1.0) -> Iterable[SampleBatch]:
        """
        :param random_seq_len: Ratio of maximum sequence lenght that will be use to generate shorter sequences if needed.
        """
        seq_len = self.max_seq_len
        if random_seq_len < 1:
            seq_len = np.random.randint(int(random_seq_len * self.max_seq_len), self.max_seq_len)
        ...
        while True:
            ...
            yield SampleBatch(
                tokens=out, loss_mask=loss_mask, values_in_seq=seq_len, seq_len=2*seq_len+2
            )
